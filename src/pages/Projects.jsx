...
export default function Projects() {
    ...
    const fetchProjects = async () => {
        ...
        try {
            ...
            const merged = ALLOWED_PROJECT_TITLES.map(allowedTitle => {
                ...
                const project = fetchedProject || fallbackProjects.find(p => normalizeTitle(p.title) === normAllowed);
                return stripGithubIfNeeded(project);
            }).filter(Boolean);
            setProjects(merged);
        } catch (remoteErr) {
    ...
    const categories = [
        ...
        { id: "andriod", name: "andriod development", count: projects.filter(p => p.category.includes("andriod")).length },
                        ...
                        <div className="space-y-1.5">
                            {categories.map(category => (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-left transition-all duration-300 border ${activeFilter === category.id
                                        ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white border-transparent shadow-md font-medium"
                                        : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 shadow-sm"
                                        }`}
                                >
                                    <span className="text-xs font-medium">{category.name}</span>
                                    <span className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-semibold ${activeFilter === category.id
